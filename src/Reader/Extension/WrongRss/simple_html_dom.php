...
function file_get_html(&$url, $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
    ...
    $redirectCount = 0;
    while (true) {
        $contents = fileGetContentsWithRedirects($url, $use_include_path, $context, $offset);
        if (!$contents) {
            return false;
        }
...
function fileGetContentsWithRedirects($url, $use_include_path, $context, $offset)
{
    ...
    if ($location && $location[0] == '/') {
        ...
        $urlHostPos = stripos($url, $urlHost);
        $urlHost = substr($url, 0, $urlHostPos + strlen($urlHost));
        $location = $urlHost . $location;
    }
    return [
        'Contents' => $contents,
        'Location' => $location,
